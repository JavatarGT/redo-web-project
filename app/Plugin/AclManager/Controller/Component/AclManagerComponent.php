...
{
    var $components = array('Auth', 'Acl', 'AclManager.AclReflector', 'Session');
	...
	public function set_session_permissions()
    {
        if(!$this->Session->check('Alaxos.Acl.permissions'))
        {
            ...
            if(!empty($user))
            {
                ...
                $actions = $this->User->query("(SELECT a1.alias || '/' || a2.alias || '/' || a3.alias AS action, arc._create AS permiso ".
					"FROM cta_acos a1 INNER JOIN cta_acos a2 ON a1.id = a2.parent_id ".
					"INNER JOIN cta_acos a3 ON a2.id = a3.parent_id  ".
					"INNER JOIN cta_aros_acos arc ON arc.aco_id = a3.id ".
					"INNER JOIN cta_aros ar ON ar.id = arc.aro_id ".
					"WHERE a1.alias = 'controllers' AND ar.model = 'Group' AND ar.foreign_key = ". $user['User']['group_id'].
					" AND a2.alias != 'AclManager' AND a2.alias != 'DebugKit' ".
					"ORDER BY a2.alias, a3.alias) ".
					"UNION ".
					"(SELECT a1.alias || '/' || a2.alias || '/' || a3.alias || '/' || a4.alias AS action, arc._create AS permiso ".
					"FROM cta_acos a1 INNER JOIN cta_acos a2 ON a1.id = a2.parent_id ".
					"INNER JOIN cta_acos a3 ON a2.id = a3.parent_id  ".
					"INNER JOIN cta_acos a4 ON a3.id = a4.parent_id  ".
					"INNER JOIN cta_aros_acos arc ON arc.aco_id = a4.id ".
					"INNER JOIN cta_aros ar ON ar.id = arc.aro_id ".
					" WHERE a1.alias = 'controllers' AND ar.model = 'Group' AND ar.foreign_key = ". $user['User']['group_id'].
					" ORDER BY a2.alias, a3.alias)");
                ...
                foreach($actions as $action)
                {
                	if ($action[0]['permiso'] == -1){
                		$permissions[$action[0]['action']] = false;	
                	}else{
                		$permissions[$action[0]['action']] = true;	
                	}
