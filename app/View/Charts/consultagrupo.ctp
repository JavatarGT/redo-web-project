...
<script type="text/javascript">
        ...
        $.ajax({async:true,
            ...
            success:function (data, textStatus) {
                ...
                $.ajax({async:true,
                    ...
                    type:'post', 
                    url:'<?php echo $this->base;?>/charts/consgrupo/2'
                });
            }, 
            type:'post', 
            url:'<?php echo $this->base;?>/charts/consgrupo/1'
        });
        $.ajax({async:true,
            data:{Consulta},
            dataType:'html', 
            success:function (data, textStatus) {
                var datos = $.parseJSON(data);
                for(var d in datos.Records){
                    series3.push([datos.Records[d].tipo,parseInt(datos.Records[d].total,10)]);
                }
                cargarGrafica1();
            }, 
            type:'post', 
            url:'<?php echo $this->base;?>/charts/consgrupo/3'
        });
    }
    function cargarGrafica(){
        var txt = 'Consultas y Reconsultas por Grupo Étnico desde ' + $('#FechaInicial').val() + ' a '+$('#FechaFinal').val();
  ...
  function cargarGrafica1(){
        var txt = 'Total de Consultas y Reconsultas por Grupo Étnico desde ' + $('#FechaInicial').val() + ' a '+$('#FechaFinal').val();
